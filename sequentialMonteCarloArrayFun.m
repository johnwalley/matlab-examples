function Ex = sequentialMonteCarloArrayFun(Np, N, resamplingThreshold, X0, s1, s2, k, th, T, ret, r)

    % Initialise particles
    x = X0(2)*parallel.gpu.GPUArray.ones(Np,1);
    x_new = parallel.gpu.GPUArray.ones(Np,1);
    w = parallel.gpu.GPUArray.ones(Np,1)/Np;

    Ex = [sum(w.*x); zeros(N,1)];

    s_2 = s1^2+s2^2;

    k2 = k^2;
    k3_2 = k*3/2;
    K2 = (k*th/4-s_2/16);
    s12 = s1^2;
    s22 = s2^2;
    s1s2 = s1*s2;

    for ii=2:N+1

        % Draw from distribution
        h = T(ii)-T(ii-1);
        sqrth = sqrt(h);
        h3_2 = h*sqrth;
        sqV = sqrt(x);

        Z1 = randn(Np, 1); Z12 = Z1.^2;
        Z2 = randn(Np, 1); Z22 = Z2.^2;
        Z1Z2 = Z1.*Z2;

        x = arrayfun(@f, x, k, th, h, sqV, sqrth, s1, Z1, s2, Z2, k2, K2, k3_2, ...
              h3_2, s12, Z12, s22, Z22, s1s2, Z1Z2);

        % Evaluate likelihood
        l = arrayfun(@normpdfGPU, ret(ii-1), r*h, sqrt(x*h));

        % Calculate modified weights
        w = l.*w;
        w = w./sum(w);

        % Calculate effective sample size
        ess = 1./sum(w.^2);

        if ess < resamplingThreshold*Np        
            a = cumsum(w);
            for n=1:Np
%                 ind = find(rand<a,1,'first');
                ind = 1;
                while rand<a(ind)
                    ind = ind + 1;
                end
                x_new(n,1) = x(ind);            
            end
            x = x_new;
            w = ones(Np,1)/Np;
        end

        Ex(ii) = sum(w.*x);

    end

end % sequentialMonteCarloArrayFun